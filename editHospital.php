...
<?php
    ...
    if (isset($_GET['id']) && is_numeric($_GET['id'])) {  // from table.php
        $id = $_GET['id'];
    } else if (isset($_GET['id_doc']) && is_numeric($_GET['id_doc'])) {  // from table.php
        $id_doc = $_GET['id_doc'];
    } else if (isset($_GET['id_pat']) && is_numeric($_GET['id_pat'])) {  // from table.php
        $id_pat = $_GET['id_pat'];
    } else {
    ...
    echo '</form>';
    echo '<a href="hospital.php?id=' . $id . '">Cancel</a>';
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $hospital_page = "http://shantim.smtchs.org/HealthOne_Medical/hospital.php?id_hos=" . $id;
